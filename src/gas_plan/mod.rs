...
pub trait GasPlan<U>: Dive<U> where U: DecoAlgorithm {
    ...
    fn calculate_consumed(segment: &DiveSegment, sac_rate: usize, metres_per_bar: f64) -> usize { // Calculate gas consumed given a segment.
        let pressure = match segment.get_segment_type() {
            AscDesc => mtr_bar(((segment.get_end_depth() + segment.get_start_depth()) / 2) as f64, metres_per_bar),
            _ => mtr_bar(segment.get_end_depth() as f64, metres_per_bar)
        };
